/* 
  Add to index.html for Firebase hosting and responsiveness:
...
const SOCIAL_LINKS = [
  ...
  { icon: Instagram, link: "https://www.instagram.com/bibash.shrestha" }, // Replace with actual profile
];
...
const Home = () => {
  ...
  useEffect(() => {
    AOS.init({
      ...
      disable: window.innerWidth < 640, // Disable AOS on small screens for performance
    });
    const handleResize = () => {
      AOS.refresh();
      if (window.innerWidth < 640) AOS.init({ disable: true });
  ...
  const lottieOptions = {
    ...
    style: {
      width: "100%",
      height: "100%",
      transform: isHovering ? "scale(1.1) rotate(2deg)" : "scale(1.0)",
