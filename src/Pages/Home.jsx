...
const SOCIAL_LINKS = [
  ...
  { icon: Instagram, link: "https://www.instagram.com/" }, // Update if you have an Instagram
];
...
const Home = () => {
  ...
  const [isLottieError, setIsLottieError] = useState(false); // Added for Lottie error handling
  ...
  useEffect(() => {
    const initAOS = () => {
      AOS.init({
        once: true,
        offset: 10,
        duration: 800, // Added duration for smooth animations
        easing: "ease-out-cubic", // Added easing for better animation flow
        disable: window.innerWidth < 640, // Disable on small screens for performance
      });
    };
    initAOS();
    const handleResize = () => initAOS(); // Refactor to handle resize
  ...
  useEffect(() => {
    let isMounted = true; // Added to prevent memory leaks
  ...
  // Lottie configuration with error handling
  const lottieOptions = {
    ...
    onError: () => setIsLottieError(true), // Added error callback
  };
                ...
                >
                  {isLottieError ? (
                    <div className="w-full h-full flex items-center justify-center bg-black/50 rounded-3xl">
                      <p className="text-gray-400 text-sm">Animation failed to load. Check network.</p>
                    </div>
                  ) : (
